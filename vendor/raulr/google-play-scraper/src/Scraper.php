...
class Scraper
{
    ...
    public function getApp($id, $lang = null, $country = null)
    {
        ...
        $crawler = $this->request(array('apps', 'details','getReviews'), $params);
        ...
        $reviewNode = $crawler->filter('.single-review');
        if ($reviewNode->count()) {
            // $info['reviews'] = implode("\n", $reviewNode->each(function ($node) {
            //     return $node->text();
            // }));



            $nlist = implode("|",$reviewNode->each(function ($node) {
                $name = $node->filter('span.author-name')->text();
                $date = $node->filter('span.review-date')->text();
                $rating = $node->filter('.star-rating-non-editable-container')->attr('aria-label');
                $comment = $node->filter('.review-body')->text();
                $link = 'https://play.google.com'. $node->filter('.review-info  .reviews-permalink')->attr('href');
